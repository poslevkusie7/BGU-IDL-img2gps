...
def plot_coords_train_val_loss(data, out_path):
    ...
    ax.set_title("Coords: Train vs Validation Loss")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.legend()
...
def plot_multitask_total_loss(data, out_path):
    epochs = data["epoch"]
    train_total = [
    ...
    ax.set_title("Multitask: Total Loss (weighted)")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.legend()
    fig.savefig(out_path, dpi=160)
...
def plot_multitask_accuracy(data, out_path):
    epochs = data["epoch"]
    plt.style.use("seaborn-v0_8-whitegrid")
    fig, ax = plt.subplots(figsize=(8, 5), constrained_layout=True)
    ax.plot(epochs, data["val_acc"], label="val_acc")
    ax.set_title("Multitask: Validation Accuracy")
    ax.set_xlabel("Epoch")
...
def plot_comparison(coords_data, multitask_data, out_path):
    plt.style.use("seaborn-v0_8-whitegrid")
    fig, axes = plt.subplots(1, 3, figsize=(14, 4), constrained_layout=True)

    ax = axes[0]
    ax.plot(coords_data["epoch"], coords_data["val_dist_m"], label="coords")
    ax.plot(multitask_data["epoch"], multitask_data["val_dist_m"], label="multitask")
    ...
    ax = axes[1]
    ax.plot(coords_data["epoch"], coords_data["p10m"], label="coords")
    ax.plot(multitask_data["epoch"], multitask_data["val_p10"], label="multitask")
    ax.set_title("P@10")
    ax.set_xlabel("Epoch")
    ...
    ax = axes[2]
    ax.plot(coords_data["epoch"], coords_data["p25m"], label="coords")
    ax.plot(multitask_data["epoch"], multitask_data["val_p25"], label="multitask")
    ax.set_title("P@25")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Fraction")
    ax.legend()

    fig.suptitle("Coords vs Multitask Comparison", fontsize=14)
    fig.savefig(out_path, dpi=160)
    plt.close(fig)
...
def main():
    ...
    multitask_path = Path("runs/multitask_log.txt")
    ...
    if not multitask_path.exists():
        print(f"Multitask log not found: {multitask_path}")
        return

    coords_data = parse_log(coords_path, COORDS_RE)
    multitask_data = parse_log(multitask_path, MULTI_RE)

    if not coords_data.get("epoch"):
        print(f"No coords epochs found in {coords_path}")
        return
    if not multitask_data.get("epoch"):
        print(f"No multitask epochs found in {multitask_path}")
        return

    plot_coords_train_val_loss(coords_data, out_dir / "coords_train_val_loss.png")
    print(f"Wrote {out_dir / 'coords_train_val_loss.png'}")

    plot_multitask_total_loss(multitask_data, out_dir / "multitask_total_loss.png")
    print(f"Wrote {out_dir / 'multitask_total_loss.png'}")

    plot_multitask_accuracy(multitask_data, out_dir / "multitask_accuracy.png")
    print(f"Wrote {out_dir / 'multitask_accuracy.png'}")

    plot_comparison(coords_data, multitask_data, out_dir / "comparison_metrics.png")
